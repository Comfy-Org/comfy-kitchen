...
__global__ void prep_sums_to_means(float* __restrict__ kc,
    ...
    if (kmean_next) kmean_next[(size_t)bh * HD + d] = total / (float)T;
}
...
// Producer path: pooled sums (written by launch_sol_producer) -> means,
// pooled stats/quant, and the routing threshold. vsc must hold the stale
// V scale TIMES 127 on entry (prep_pooled_stats divides it back down).
extern "C" void launch_sol_finish(
    void* kc_sums, void* kciP, void* kcs, void* vsc, void* threshold,
    const void* cen8, const void* cens, void* kmean_next, void* scratch_stats,
    int B, int T, int H, int NTB, int NPAD, int NQ,
    float tau, float scale_log2, cudaStream_t stream)
{
    float* kmean = (float*)scratch_stats;
    float* kcvar = kmean + (size_t)B * H * HD;
    prep_sums_to_means<<<B * H, HD, 0, stream>>>(
        (float*)kc_sums, (float*)kmean_next, T, NTB, NPAD);
    prep_pooled_stats<<<B * H, HD, 0, stream>>>(
        (float*)kc_sums, kmean, (float*)vsc, kcvar, NTB, NPAD);
    prep_pooled_quant<<<dim3(NPAD, B * H), HD, 0, stream>>>(
        (float*)kc_sums, kmean, (int8_t*)kciP, (float*)kcs, NTB, NPAD);
    prep_thr_from_cen<<<dim3(NQ, B * H), HD, 0, stream>>>(
        (const int8_t*)cen8, (const float*)cens, kcvar, (float*)threshold,
        NQ, tau, scale_log2);
}
