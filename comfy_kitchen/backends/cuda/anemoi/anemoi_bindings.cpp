...
// CMake fills COMFY_ANEMOI_SM89_ARCHS with the comma-separated capability
...
#if defined(COMFY_ANEMOI_SM89) && !defined(COMFY_ANEMOI_SM89_ARCHS)
#define COMFY_ANEMOI_SM89_ARCHS "89"
#endif

bool compiled(int architecture) {
#ifdef COMFY_ANEMOI_SM89_ARCHS
  for (const char *p = COMFY_ANEMOI_SM89_ARCHS; *p;) {
    int value = 0;
    while (*p >= '0' && *p <= '9')
      value = value * 10 + (*p++ - '0');
...
DType scalar(nb::dlpack::dtype d) {
  ...
  throw std::invalid_argument("unsupported Anemoi boundary dtype");
}
Tensor tensor(const Array &a, const Config &c, const char *what, bool indexed = true) {
  ...
  if (t.numel()) {
    size_t width = anemoi_native::itemsize(t.scalar_type());
    check(span < SIZE_MAX / width, what, ": tensor byte span overflow");
    cudaPointerAttributes attributes{};
    anemoi_native::cuda_check(cudaPointerGetAttributes(&attributes, t.pointer));
...
void expect(const Tensor &t, anemoi_native::IntArrayRef shape, DType dtype,
            bool contiguous = true) {
  check(t.shape == shape && t.scalar_type() == dtype && (!contiguous || t.is_contiguous()),
        "Anemoi tensor shape, dtype or layout does not match plan");
}
...
Plan plan(int device, int batch, int tokens, int heads, int dim, int block,
  ...
  const cudaDeviceProp &properties = *anemoi_native::cuda::deviceProperties(device);
  Config c;
  c.device = device;
  ...
  validate(c);
  check(compiled(c.architecture), "Anemoi architecture was not compiled");
...
void launch(const Plan &p, Array q, Array k, Array v, Array indices, Array valid,
  ...
  anemoi_native::cuda::CUDAGuard guard(c.device);
  cudaStream_t stream = reinterpret_cast<cudaStream_t>(stream_value);
  if (stream) {
    int stream_device;
    anemoi_native::cuda_check(cudaStreamGetDevice(stream, &stream_device));
    check(stream_device == c.device, "Anemoi stream belongs to another device");
  }
  Inputs in;
  ...
  check(storage.scalar_type() == DType::Byte && storage.dim() == 1 &&
            ...
            uintptr_t(storage.pointer) % 256 == 0, "invalid Anemoi workspace");
  ...
  auto overlap = [](const Tensor &a, const Tensor &b) {
    ...
    auto end = [](const Tensor &t) {
      ...
      return uintptr_t(t.pointer) + span * anemoi_native::itemsize(t.scalar_type());
    };
  ...
  for (const Tensor *t : {&in.q, &in.k, &in.v, &in.indices, &in.slot_valid,
                          ...
                          &in.prefix_output}) {
    check(!overlap(*t, storage) && !overlap(*t, in.output), "Anemoi writable storage aliases input");
  }
...
void register_anemoi(nb::module_ &m) {
  nb::class_<Plan>(m, "_AnemoiPlan")
      .def_ro("workspace_bytes", &Plan::workspace_bytes)
      .def_ro("native_prefix", &Plan::native_prefix);
  m.def("anemoi_supports_arch", &compiled);
  m.def("anemoi_plan", &plan, nb::arg("device"), nb::arg("batch"), nb::arg("tokens"),
  ...
  m.def("anemoi", &launch, nb::arg("plan"), nb::arg("q"), nb::arg("k"), nb::arg("v"),
