...
// Native scheduling of the validated Anemoi SM89 H3 kernels.
...
using Shape = anemoi_native::IntArrayRef;
...
class KernelContext {
  anemoi_native::ExecutionContext context_;
  anemoi_native::ExecutionContext *previous_;
public:
  KernelContext(int device, cudaStream_t stream,
                std::initializer_list<Tensor> buffers)
      : previous_(anemoi_native::active_context) {
    context_.device = device;
    context_.stream = stream;
    context_.buffers = buffers;
    anemoi_native::active_context = &context_;
  }
  ~KernelContext() { anemoi_native::active_context = previous_; }
  void finish() const {
...
struct Plan {
  ...
  explicit Plan(const Config &c) {
    ...
    route_scratch = add({anemoi_native::sm89_route_workspace_bytes(
...
void execute_sm89(const Config &c, const Inputs &in, void *workspace,
  ...
  anemoi_native::cuda::CUDAGuard device_guard(c.device);
