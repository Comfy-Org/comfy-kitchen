...
using Tensor = anemoi_native::Tensor;
using DType = anemoi_native::ScalarType;
...
struct Workspace {
  size_t bytes = 0;
  Region add(anemoi_native::IntArrayRef shape, DType dtype, int device);
};
...
// Native shared peripheral kernels (implemented in anemoi_preprocess.cu).
