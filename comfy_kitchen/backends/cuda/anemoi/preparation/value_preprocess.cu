...
std::tuple<anemoi_native::Tensor, anemoi_native::Tensor> preprocess_v_fp8(
    anemoi_native::Tensor value) {
  ANEMOI_CHECK(value.is_cuda(), "value must be a CUDA tensor");
  ANEMOI_CHECK(value.is_contiguous(), "value must be contiguous HND");
  ANEMOI_CHECK(value.scalar_type() == anemoi_native::ScalarType::Half,
              "value must be FP16");
  ANEMOI_CHECK(value.dim() == 4, "value must have shape [B,Hkv,K,D]");
  ANEMOI_CHECK(
      value.size(0) > 0 && value.size(1) > 0 && value.size(2) > 0,
      "value batch, head, and token dimensions must be positive");
  ANEMOI_CHECK(value.size(3) == 64 || value.size(3) == 128,
  ...
  auto permuted = anemoi_native::empty(
  ...
  auto value_fp8 = anemoi_native::empty(
      ...
      value.options().dtype(anemoi_native::ScalarType::Float8_e4m3fn));
  auto value_scale = anemoi_native::empty(
      {batch_size, num_heads, head_dim},
      value.options().dtype(anemoi_native::ScalarType::Float));

  cudaStream_t stream = anemoi_native::getCurrentCUDAStream();
  const auto* input =
      reinterpret_cast<const half*>(value.data_ptr<anemoi_native::Half>());
  auto* intermediate =
      reinterpret_cast<half*>(permuted.data_ptr<anemoi_native::Half>());
  if (head_dim == 64) {
...
std::tuple<anemoi_native::Tensor, anemoi_native::Tensor> quantize_permuted_v_fp8(
    anemoi_native::Tensor permuted_value) {
  ANEMOI_CHECK(
  ...
  ANEMOI_CHECK(
      permuted_value.scalar_type() == anemoi_native::ScalarType::Half,
  ...
  anemoi_native::cuda::CUDAGuard device_guard(permuted_value.device());
  ...
  auto value_scale = anemoi_native::empty(
      {batch_size, num_heads, head_dim},
      permuted_value.options().dtype(anemoi_native::ScalarType::Float));
  ...
  mpa::attention::quantize_v_per_channel_kernel<<<
      quant_grid, mpa::attention::kQuantThreads, 0, stream>>>(
      reinterpret_cast<const half*>(permuted_value.data_ptr<anemoi_native::Half>()),
      reinterpret_cast<__nv_fp8_e4m3*>(value_fp8.data_ptr()),
...
quantize_permuted_v_fp8_h3_vpartials(
    anemoi_native::Tensor permuted_value,
    anemoi_native::Tensor value_stage_amax) {
  ...
  ANEMOI_CHECK(
      permuted_value.is_contiguous() &&
          permuted_value.scalar_type() == anemoi_native::ScalarType::Half &&
          permuted_value.sizes() == anemoi_native::IntArrayRef(
              {kH3Batch, kH3Heads, kH3HeadDim, kH3VirtualTokens}),
      "private H3 permuted V must be contiguous FP16 "
      "[1,14,128,42624]");
  ANEMOI_CHECK(
      value_stage_amax.is_contiguous() &&
          value_stage_amax.scalar_type() == anemoi_native::ScalarType::Float &&
          value_stage_amax.sizes() == anemoi_native::IntArrayRef(
...
quantize_packed_v_fp8_h3_vpartials(
  ...
  ANEMOI_CHECK(
      packed_value.is_contiguous() &&
          packed_value.scalar_type() == anemoi_native::ScalarType::Half &&
          packed_value.sizes() == anemoi_native::IntArrayRef(
  ...
  anemoi_native::cuda::CUDAGuard device_guard(packed_value.device());
  const cudaDeviceProp* properties =
      anemoi_native::cuda::getCurrentDeviceProperties();
  ANEMOI_CHECK(
      properties->major == 8 && properties->minor == 9,
  ...
  auto value_scale = anemoi_native::empty(
      {kH3Batch, kH3Heads, kH3HeadDim},
      packed_value.options().dtype(anemoi_native::ScalarType::Float));
  const cudaStream_t stream =
      anemoi_native::getCurrentCUDAStream();
  ...
  mpa::attention::quantize_packed_v_fp8_h3_vpartials_kernel<<<
      quant_grid, mpa::attention::kQuantThreads, 0, stream>>>(
      reinterpret_cast<const half*>(packed_value.data_ptr<anemoi_native::Half>()),
      value_stage_amax.data_ptr<float>(),
      reinterpret_cast<__nv_fp8_e4m3*>(value_fp8.data_ptr()),
      value_scale.data_ptr<float>());
  ANEMOI_CUDA_KERNEL_LAUNCH_CHECK();
