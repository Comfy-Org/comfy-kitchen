...
using anemoi_native::getCurrentCUDAStream;
// cudaGetDeviceProperties costs over a millisecond on current drivers and the
// Anemoi launch path queries it once per phase (preparation, draft, routing,
...
inline const cudaDeviceProp *deviceProperties(int device) {
  ...
  if (device < 0)
    throw std::invalid_argument("Anemoi queried a negative CUDA device ordinal");
...
#define ANEMOI_CUDA_CHECK(expr) ::anemoi_native::cuda_check(expr)
#define ANEMOI_CUDA_KERNEL_LAUNCH_CHECK()                                      \
  do {                                                                         \
    ANEMOI_CUDA_CHECK(cudaGetLastError());                                     \
  } while (0)
