...
Region Workspace::add(draft_native::IntArrayRef shape, DType dtype, int device) {
  Tensor t;
  ...
  size_t length = size_t(t.numel()) * draft_native::itemsize(dtype);
...
void validate(const Config &c) {
  check(ada_serves(c.architecture) || c.architecture == 120,
        "Draft requires SM89 or newer (SM120+ uses the Blackwell kernels)");
  check(c.device >= 0 && c.batch == 1 && c.tokens > 0 && c.heads > 0,
        "native Draft requires batch one and positive tensor dimensions");
  ...
  check(c.input_type == DType::Half || c.input_type == DType::BFloat16,
        "Draft input must be FP16 or BF16");
...
namespace {
struct Outputs {
  draft_native::ExecutionContext context;
  draft_native::ExecutionContext *previous;
  Outputs(int device, cudaStream_t stream, std::vector<Tensor> buffers)
      : previous(draft_native::active_context) {
    context.device = device;
    context.stream = stream;
    context.buffers = std::move(buffers);
    draft_native::active_context = &context;
  }
  ~Outputs() { draft_native::active_context = previous; }
};
}
