...
inline int require_nonnegative_int32(int64_t value, const char* name) {
  DRAFT_CHECK(value >= 0, name, " must be nonnegative");
  DRAFT_CHECK(
...
route_precision_impl(
  ...
  DRAFT_CHECK(
      probability.is_cuda() &&
          ...
          probability.scalar_type() == draft_native::ScalarType::Half,
  ...
  DRAFT_CHECK(keep_value > 0, "retained count must be positive");
  ...
  if (anchors.has_value()) {
    DRAFT_CHECK(
        anchors->is_cuda() && anchors->is_contiguous() &&
            anchors->scalar_type() == draft_native::ScalarType::Bool &&
    ...
    DRAFT_CHECK(
        anchor_ids.has_value() && anchor_ids->is_cuda() && anchor_ids->is_contiguous() &&
            anchor_ids->scalar_type() == draft_native::ScalarType::Int &&
  ...
  DRAFT_CHECK(keep_value <= segment_items_value, "retained count exceeds R*R");
  DRAFT_CHECK(
  ...
  DRAFT_CHECK(segments_value <= kMaxSegments, "B*H exceeds global route key");
  ...
  auto int_options = probability.options().dtype(draft_native::ScalarType::Int);
  auto byte_options = probability.options().dtype(draft_native::ScalarType::Byte);
  auto precision_map = draft_native::empty(assembly_route_draft_native::shape(probability), byte_options);
  auto block_ids = draft_native::empty(assembly_route_draft_native::shape(probability), int_options);
  auto sorted_ids = draft_native::empty_like(block_ids);
  auto input_keys = draft_native::empty(assembly_route_draft_native::shape(probability), int_options);
  auto sorted_keys = draft_native::empty_like(input_keys);
  auto low_counts = draft_native::empty(
      {probability.size(0), probability.size(1), rows}, int_options);
  auto middle_counts = draft_native::empty_like(low_counts);
  auto high_counts = draft_native::empty_like(low_counts);
  const cudaStream_t stream =
      draft_native::cuda::getCurrentCUDAStream();

  const int64_t blocks = (total_items_value + kThreads - 1) / kThreads;
  DRAFT_CHECK(blocks <= kMaxGridX, "route grid.x exceeds CUDA limit");
  ...
  const int64_t required_workspace = draft_native::sm89_route_workspace_bytes(
      probability.size(0), probability.size(1), rows, probability.device(), stream);
  auto workspace = draft_native::empty({required_workspace}, byte_options);
  ...
  DRAFT_CHECK(row_count_value <= kMaxGridX, "row-pack grid.x exceeds CUDA limit");
...
materialize_route_impl(
  ...
  DRAFT_CHECK(
      logical_ids.is_cuda() &&
          ...
          logical_ids.scalar_type() == draft_native::ScalarType::Int &&
          logical_ids.dim() == 4,
      "logical_ids must be contiguous CUDA int32 [B,H,R,C]");
  const auto check_counts = [&](const draft_native::Tensor& counts, const char* name) {
    DRAFT_CHECK(
        counts.is_cuda() && counts.is_contiguous() &&
            counts.scalar_type() == draft_native::ScalarType::Int &&
  ...
  auto physical_ids = draft_native::empty(physical_sizes, logical_ids.options());
  auto physical_low_counts = draft_native::empty_like(low_counts);
  auto physical_middle_counts = draft_native::empty_like(middle_counts);
  auto physical_high_counts = has_high
      ? draft_native::empty_like(high_counts)
      : draft_native::empty({0}, high_counts.options());
  const cudaStream_t stream =
      draft_native::cuda::getCurrentCUDAStream();
...
sm89_h3_route_precision(
    draft_native::Tensor probability, int64_t n16, int64_t n8, int64_t n4,
    std::optional<draft_native::Tensor> anchors,
    std::optional<draft_native::Tensor> anchor_ids, int64_t anchor_count) {
...
sm89_h3_materialize_route(
    draft_native::Tensor logical_ids, draft_native::Tensor low_counts,
    draft_native::Tensor middle_counts, draft_native::Tensor high_counts,
  ...
  DRAFT_CHECK(draft_native::ada_serves_device(&properties),
...
int64_t draft_native::sm89_route_workspace_bytes(
