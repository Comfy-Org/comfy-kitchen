...
namespace {
namespace prep = ::draft::sm120::preparation;
using draft_native::check;
using draft_native::cuda_check;
...
TensorView attention_view(const Tensor &t) {
  ...
  using S = draft_sm120::ScalarType;
  switch (t.scalar_type()) {
...
struct Plan {
  ...
  explicit Plan(const Config &c) {
    ...
    const auto add = [&](draft_native::IntArrayRef shape, DType type, bool enabled = true) {
      return workspace.add(enabled ? shape : draft_native::IntArrayRef{0}, type, c.device);
    };
...
void execute_sm120(const Config &c, const Inputs &in, void *workspace,
  ...
  if (c.jensen) {
    ...
    draft_sm120::launch_k_tail_probability(t[0].pointer,t[1].pointer,t[3].pointer,
    ...
    draft_sm120::launch_draft_probability(t[0].pointer,t[1].pointer,
