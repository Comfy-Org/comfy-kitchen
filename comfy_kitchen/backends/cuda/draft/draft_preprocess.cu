...
Packed packed_view(const Tensor &t, const Tensor &counts, int block) {
  draft_native::check(t.dim() == 4 && t.scalar_type() == DType::Half &&
                          (block == 64 || block == 128) &&
...
void valid_key_counts(const Config &c, const Tensor &logical, const Tensor &physical,
  ...
  draft_native::check(physical.numel() == n, "physical K64 count capacity mismatch");
  ...
  draft_native::cuda_check(cudaGetLastError());
}
