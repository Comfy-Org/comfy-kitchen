...
inline void check_output(const TensorView &out, const TensorView &q,
  ...
  const cudaDeviceProp &prop = *draft_native::cuda::deviceProperties(current);
  check(prop.major == 12 && prop.minor == 0,
        "Draft SM120 requires exact compute capability 12.0");
  int stream_device = -1;
...
#define DRAFT_SM120_CHECK(...) ::draft_sm120::check(__VA_ARGS__)
#define DRAFT_SM120_CUDA_CHECK(expr) ::draft_sm120::cuda_check(expr)
