...
cublasHandle_t draft_handle(int device, cudaStream_t stream) {
  struct HandleCache {
    const DraftCublasFunctions *api = nullptr;
  ...
  const auto &api = comfy::draft_cublas();
...
void product(cublasHandle_t handle, const DraftScratch &s,
  ...
  blas_check(comfy::draft_cublas().gemm_strided_batched_ex(
...
void draft_probability(const Tensor &q_pool, const Tensor &k_pool,
  ...
  check(probability.defined() && probability.pointer && probability.is_contiguous() &&
        ...
        probability.sizes() == draft_native::IntArrayRef({q_pool.size(0),q_pool.size(1),s.rows,s.rows}),
  ...
  draft_native::cuda::CUDAGuard guard(q_pool.device());
