...
void check_raw_hnd(const draft_native::Tensor& tensor, const char* name) {
  DRAFT_CHECK(tensor.defined(), name, " must be defined");
  ...
  DRAFT_CHECK(tensor.dim() == 4, name, " must have shape [B,H,F*Y*X,D]");
  DRAFT_CHECK(
...
Raster8x16Layout checked_layout(
    ...
    int64_t height,
    int64_t width) {
  DRAFT_CHECK(frames > 0, "frames must be positive");
  DRAFT_CHECK(height > 0, "height must be positive");
  DRAFT_CHECK(width > 0, "width must be positive");
  DRAFT_CHECK(
...
void check_raster_and_grid(
    const draft_native::Tensor& anchor,
    int64_t patch_count,
    int64_t max_heads) {
  const draft_native::cuda::CUDAGuard properties_guard(anchor.device());
...
void launch_raster_pack(
    const draft_native::Tensor& query,
    const draft_native::Tensor& key,
    const draft_native::Tensor& value,
    draft_native::Tensor& packed_query,
    draft_native::Tensor& packed_key,
  ...
  pack_raster_operand_to_fp16_kernel<HeadDim, InputT><<<
      ...
      reinterpret_cast<half*>(packed_query.data_ptr<draft_native::Half>()),
      layout,
      query_heads,
      raw_tokens,
      virtual_tokens,
  ...
  pack_raster_kv_to_fp16_kernel<HeadDim, InputT><<<
      ...
      reinterpret_cast<half*>(packed_key.data_ptr<draft_native::Half>()),
      reinterpret_cast<half*>(packed_value.data_ptr<draft_native::Half>()),
      layout,
      kv_heads,
      raw_tokens,
      virtual_tokens,
...
std::tuple<draft_native::Tensor, draft_native::Tensor> prepare_sm89_prefix_q_int8(
  ...
  auto output = draft_native::empty(
      ...
      query.options().dtype(draft_native::ScalarType::Char));
  auto scales = draft_native::empty(
      {query.size(0), query.size(1), prefix_blocks},
      query.options().dtype(draft_native::ScalarType::Float));
  const dim3 grid(
  ...
  if (query.scalar_type() == draft_native::ScalarType::Half) {
    if (query.size(3) == 64) {
...
quantize_sm89_qk_int8(
  ...
  DRAFT_CHECK(
      query.scalar_type() == draft_native::ScalarType::Half &&
          key.scalar_type() == draft_native::ScalarType::Half,
  ...
  if (key_mean.has_value()) {
    const draft_native::Tensor& mean = *key_mean;
    DRAFT_CHECK(
        mean.is_cuda() && mean.device() == key.device() &&
            mean.scalar_type() == draft_native::ScalarType::Half &&
            mean.is_contiguous(),
  ...
  auto q8 = draft_native::empty_like(query, query.options().dtype(draft_native::ScalarType::Char));
  auto k8 = draft_native::empty_like(key, key.options().dtype(draft_native::ScalarType::Char));
  auto q_scale = draft_native::empty(
      {query.size(0), query.size(1), query_blocks},
      query.options().dtype(draft_native::ScalarType::Float));
  auto k_scale = draft_native::empty(
      {key.size(0), key.size(1), key_blocks},
      key.options().dtype(draft_native::ScalarType::Float));
  const cudaStream_t stream =
      draft_native::getCurrentCUDAStream();
  ...
  const auto launch_query = [&](auto head_dim_tag, auto block_tag) {
    ...
    quantize_sm89_contiguous_int8_kernel<HeadDim, BlockTokens, false><<<
        grid, 256, 0, stream>>>(
        reinterpret_cast<const half*>(query.data_ptr<draft_native::Half>()), nullptr,
  ...
  const auto launch_key = [&](auto head_dim_tag, auto subtract_mean_tag) {
    ...
    const half* mean = key_mean.has_value()
        ? reinterpret_cast<const half*>(key_mean->data_ptr<draft_native::Half>())
        : nullptr;
    quantize_sm89_contiguous_int8_kernel<HeadDim, 64, SubtractMean><<<
        grid, 256, 0, stream>>>(
        reinterpret_cast<const half*>(key.data_ptr<draft_native::Half>()), mean,
...
pack_indexed_k64_qkv_fp16(
  ...
  DRAFT_CHECK(
      token_indices.is_cuda() && token_indices.device() == query.device() &&
          token_indices.scalar_type() == draft_native::ScalarType::Long &&
  ...
  DRAFT_CHECK(
      slot_valid.is_cuda() && slot_valid.device() == query.device() &&
          slot_valid.scalar_type() == draft_native::ScalarType::Bool &&
  ...
  const auto fp16_options = query.options().dtype(draft_native::ScalarType::Half);
  auto packed_query = draft_native::empty(
      {query.size(0), query.size(1), physical_tokens, query.size(3)},
      fp16_options);
  auto packed_key = draft_native::empty_like(packed_query);
  auto packed_value = draft_native::empty_like(packed_query);
  const dim3 grid(
...
pack_h3_k64_qkv_fp16(
  ...
  DRAFT_CHECK(
      video_token_indices.is_cuda() &&
          ...
          video_token_indices.scalar_type() == draft_native::ScalarType::Long &&
  ...
  DRAFT_CHECK(
      video_slot_valid.is_cuda() &&
          ...
          video_slot_valid.scalar_type() == draft_native::ScalarType::Bool &&
  ...
  auto packed_query = draft_native::empty(
      ...
      fp16_options);
  auto packed_key = draft_native::empty(
      {query.size(0), query.size(1), key_physical_tokens, query.size(3)},
      fp16_options);
  auto packed_value = draft_native::empty_like(packed_key);
  const dim3 grid(
...
H3SM89Int8Prepared prepare_h3_sm89_int8_operands(
  ...
  DRAFT_CHECK(
      video_valid_counts.is_cuda() &&
          ...
          video_valid_counts.scalar_type() == draft_native::ScalarType::Int &&
  ...
  draft_native::cuda::CUDAGuard device_guard(query.device());
  const auto fp16_options = query.options().dtype(draft_native::ScalarType::Half);
  const auto int8_options = query.options().dtype(draft_native::ScalarType::Char);
  const auto fp32_options = query.options().dtype(draft_native::ScalarType::Float);
  auto q_pool = draft_native::empty(
      {query.size(0), query.size(1), video_blocks, head_dim}, fp16_options);
  auto k_pool = draft_native::empty_like(q_pool);
  auto q_max_pool = has_maxpool ? draft_native::empty_like(q_pool)
                                : draft_native::empty({0}, fp16_options);
  auto k_max_pool = has_maxpool ? draft_native::empty_like(q_pool)
                                : draft_native::empty({0}, fp16_options);
  auto packed_q = draft_native::empty(
      {query.size(0), query.size(1), video_physical_tokens, head_dim},
      fp16_options);
  auto packed_k = draft_native::empty(
      {query.size(0), query.size(1), key_physical_tokens, head_dim},
      fp16_options);
  auto packed_v = draft_native::empty_like(packed_k);
  auto q8 = draft_native::empty(packed_q.sizes(), int8_options);
  auto k8 = draft_native::empty(packed_k.sizes(), int8_options);
  auto v8 = draft_native::empty(
      {query.size(0), query.size(1), head_dim, padded_key_tokens},
      query.options().dtype(draft_native::ScalarType::Float8_e4m3fn));
  auto q_scale = draft_native::empty(
      {query.size(0), query.size(1), video_blocks}, fp32_options);
  auto k_scale = draft_native::empty(
      {query.size(0), query.size(1), physical_stages}, fp32_options);
  auto v_scale = draft_native::empty(
      {query.size(0), query.size(1), head_dim}, fp32_options);
  auto k_mean = smooth_k
      ? draft_native::empty({query.size(0), query.size(1), head_dim}, fp16_options)
      : draft_native::empty({0}, fp16_options);
  auto k_stage_sum = smooth_k
      ? draft_native::empty(
            {query.size(0), query.size(1), physical_stages, head_dim},
            fp32_options)
      : draft_native::empty({0}, fp32_options);
  auto v_stage_amax = draft_native::empty(
      {query.size(0), query.size(1), physical_stages, head_dim},
      fp32_options);
  ...
  const auto launch = [&](auto head_dim_tag, auto query_block_tag, auto* input_tag) {
    ...
    const auto launch_qk = [&](auto smooth_tag, auto maxpool_tag) {
      ...
      prepare_h3_sm89_qk_single_load_kernel<
          ...
          reinterpret_cast<half*>(q_pool.data_ptr<draft_native::Half>()),
          reinterpret_cast<half*>(k_pool.data_ptr<draft_native::Half>()),
          HasMaxPool
              ? reinterpret_cast<half*>(q_max_pool.data_ptr<draft_native::Half>())
              : nullptr,
          HasMaxPool
              ? reinterpret_cast<half*>(k_max_pool.data_ptr<draft_native::Half>())
              : nullptr,
          reinterpret_cast<half*>(packed_q.data_ptr<draft_native::Half>()),
          reinterpret_cast<half*>(packed_k.data_ptr<draft_native::Half>()),
          q8.data_ptr<int8_t>(),
          k8.data_ptr<int8_t>(),
    ...
    prepare_h3_sm89_v_single_load_kernel<InputT, HeadDim><<<
        ...
        video_token_indices.data_ptr<int64_t>(),
        video_slot_valid.data_ptr<bool>(),
        reinterpret_cast<half*>(packed_v.data_ptr<draft_native::Half>()),
    ...
    if (smooth_k) {
      ...
      reduce_h3_sm89_k_mean_kernel<HeadDim><<<
          ...
          reinterpret_cast<half*>(k_mean.data_ptr<draft_native::Half>()),
          key.size(1), physical_stages, key.size(2));
      DRAFT_CUDA_KERNEL_LAUNCH_CHECK();
      quantize_h3_sm89_smoothed_k_kernel<HeadDim, QueryBlock><<<
          v_grid, HeadDim, 0, stream>>>(
          reinterpret_cast<const half*>(packed_k.data_ptr<draft_native::Half>()),
          reinterpret_cast<const half*>(k_mean.data_ptr<draft_native::Half>()),
          video_valid_counts.data_ptr<int32_t>(),
          k8.data_ptr<int8_t>(), k_scale.data_ptr<float>(),
  ...
  const auto launch_v_quant = [&](auto head_dim_tag) {
    ...
    prepare_h3_sm89_v_from_partials_kernel<HeadDim><<<
        ...
        reinterpret_cast<const half*>(packed_v.data_ptr<draft_native::Half>()),
        v_stage_amax.data_ptr<float>(),
...
draft_native::Tensor pack_raster_v_fp16(
  ...
  auto packed_value = draft_native::empty(
      {value.size(0), value.size(1), virtual_tokens, value.size(3)},
      value.options().dtype(draft_native::ScalarType::Half));
  const dim3 grid(
      static_cast<unsigned int>(layout.patch_count),
  ...
  if (value.scalar_type() == draft_native::ScalarType::Half) {
    if (value.size(3) == 64) {
      launch(std::integral_constant<int32_t, 64>{},
             static_cast<half*>(nullptr));
    } else {
...
std::tuple<draft_native::Tensor, draft_native::Tensor> pack_raster_kv_fp16(
  ...
  draft_native::cuda::CUDAGuard device_guard(key.device());
  check_raster_and_grid(key, layout.patch_count, key.size(1));
  const auto fp16_options = key.options().dtype(draft_native::ScalarType::Half);
  auto packed_key = draft_native::empty(
      {key.size(0), key.size(1), virtual_tokens, key.size(3)},
      fp16_options);
  auto packed_value = draft_native::empty_like(packed_key);
  ...
  if (key.scalar_type() == draft_native::ScalarType::Half) {
    if (key.size(3) == 64) {
      launch(std::integral_constant<int32_t, 64>{},
             static_cast<half*>(nullptr));
    } else {
