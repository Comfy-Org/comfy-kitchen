...
// Optional classic cuBLAS for Draft DraftMap. No cuBLAS import-library ABI.
#pragma once
...
class DraftCublasRuntime final {
public:
    static const DraftCublasRuntime& instance() {
        ...
        static const auto* runtime = new DraftCublasRuntime();
        return *runtime;
    }
    ...
    const DraftCublasFunctions& functions() const {
        if (!is_available()) throw std::runtime_error(error_);
        return functions_;
    }

    DraftCublasRuntime(const DraftCublasRuntime&) = delete;
    DraftCublasRuntime& operator=(const DraftCublasRuntime&) = delete;
...
private:
    ...
    static bool symbols(Library library, DraftCublasFunctions& api,
                        std::string& error) {
    ...
    DraftCublasRuntime() {
        ...
        error_ = "Draft DraftMap requires an available classic cuBLAS 12.x or 13.x runtime "
                 ...
                 "operators do not require this optional Draft dependency. Tried:" + failures;
    }

    Library library_ = nullptr;
    DraftCublasFunctions functions_;
    std::string error_;
};

// Call only from an Draft draft invocation, never module registration/static
...
inline const DraftCublasFunctions& draft_cublas() {
    return DraftCublasRuntime::instance().functions();
}
