...
using namespace draft_sm120;
#include "q64_attention_decl.cuh"

namespace {

void check_cuda_contiguous(const TensorView& tensor, const char* name) {
  DRAFT_SM120_CHECK(tensor.is_cuda(), name, " must be a CUDA tensor");
  DRAFT_SM120_CHECK(tensor.is_contiguous(), name, " must be contiguous");
  for (auto d : tensor.shape) DRAFT_SM120_CHECK(d >= 0 && d <= UINT32_MAX, name, " dimension exceeds uint32");
  DRAFT_SM120_CHECK(tensor.numel() <= UINT32_MAX, name, " exceeds donor uint32 indexing capacity");
  if (tensor.dim() == 4) DRAFT_SM120_CHECK(tensor.size(0) > 0 && tensor.size(1) > 0, name, " batch/heads must be positive");
}
...
void fp16_attention_forward(
  ...
  DRAFT_SM120_CHECK(query.dim() == 4, "query must have shape [B,Hq,Q,D]");
  DRAFT_SM120_CHECK(key.dim() == 4 && value.dim() == 4,
              "key/value must have shape [B,Hkv,K,D]");
  DRAFT_SM120_CHECK(query.scalar_type() == ScalarType::Half,
              "query must be FP16");
  DRAFT_SM120_CHECK(key.scalar_type() == ScalarType::Half,
              "key must be FP16");
  DRAFT_SM120_CHECK(value.scalar_type() == ScalarType::Half,
              "value must be FP16");
  DRAFT_SM120_CHECK(key.sizes() == value.sizes(), "key/value shapes must match");
  DRAFT_SM120_CHECK(query.size(0) > 0 && query.size(1) > 0 && query.size(2) > 0,
              "query dimensions must be positive");
  if constexpr (QueryBlock == 128) {
    DRAFT_SM120_CHECK(query.size(2) % 128 == 0, "Q must be a multiple of 128");
  }
  DRAFT_SM120_CHECK(key.size(0) == query.size(0), "query/key batch mismatch");
  DRAFT_SM120_CHECK(key.size(2) > 0 && key.size(2) % 64 == 0,
              "K must be a positive multiple of 64 physical slots");
  DRAFT_SM120_CHECK((query.size(3) == 64 || query.size(3) == 128) && key.size(3) == query.size(3),
              "native K64 attention requires head_dim in {64,128}");
  DRAFT_SM120_CHECK(query.size(1) % key.size(1) == 0,
              "query heads must be divisible by KV heads");
  DRAFT_SM120_CHECK(
      std::isfinite(softmax_scale) && softmax_scale > 0.0,
      "softmax_scale must be finite and positive");

  DRAFT_SM120_CHECK(block_ids.scalar_type() == ScalarType::Int,
              "block_ids must be int32");
  DRAFT_SM120_CHECK(block_counts.scalar_type() == ScalarType::Int,
              "block_counts must be int32");
  DRAFT_SM120_CHECK(valid_k_counts.scalar_type() == ScalarType::Int,
  ...
  DRAFT_SM120_CUDA_CHECK(cudaGetLastError());
  return;
}
...
void int8_attention_forward(
  ...
  DRAFT_SM120_CHECK(k16.sizes() == v16.sizes(), "k16/v16 shapes must match");
  DRAFT_SM120_CHECK(
  ...
  DRAFT_SM120_CHECK(q8.sizes() == q16.sizes(), "q8 must match q16 shape");
  DRAFT_SM120_CHECK(k8.sizes() == k16.sizes(), "k8 must match k16 shape");
  DRAFT_SM120_CHECK(
...
void mxfp8_attention_forward(
  ...
  DRAFT_SM120_CHECK(k_fp16.sizes() == v_fp16.sizes(), "FP16 K/V shapes must match");
  DRAFT_SM120_CHECK(
  ...
  DRAFT_SM120_CHECK(q_mxfp8.sizes() == q_fp16.sizes(), "q_mxfp8 must match q_fp16");
  DRAFT_SM120_CHECK(k_mxfp8.sizes() == k_fp16.sizes(), "k_mxfp8 must match k_fp16");
  DRAFT_SM120_CHECK(
  ...
  DRAFT_SM120_CHECK(block_ids.scalar_type() == ScalarType::Int,
              "block_ids must be int32");
  DRAFT_SM120_CHECK(mxfp8_block_counts.scalar_type() == ScalarType::Int,
              "mxfp8_block_counts must be int32");
  DRAFT_SM120_CHECK(fp16_block_counts.scalar_type() == ScalarType::Int,
  ...
  const auto launch = [&](auto head_dim_tag) {
    ...
    if constexpr (CompactSequential) {
      ...
      DRAFT_SM120_CHECK(!active_fp16, "compact MXFP8 is a pure compute ceiling");
...
void nvfp4_attention_forward(
  ...
  for (const auto& item : {
           ...
           std::pair<const TensorView*, const char*>(&v_nvfp4, "v_nvfp4"),
           std::pair<const TensorView*, const char*>(&v_nvfp4_scale, "v_nvfp4_scale")}) {
    DRAFT_SM120_CHECK(item.first->scalar_type() == ScalarType::Byte,
                item.second, " must be uint8");
  }
  DRAFT_SM120_CHECK(
      q_nvfp4.sizes() == Shape(
  ...
  DRAFT_SM120_CHECK(
      active_fp16 || fp16_prefix_blocks == 0,
      "inactive FP16 requires zero prefix stages");
  DRAFT_SM120_CHECK(std::isfinite(softmax_scale) && softmax_scale > 0.0,
              "softmax_scale must be finite and positive");
