...
template <typename T> __forceinline__ __device__ float to_float(T v) {
...
__forceinline__ __device__ void store4_i8(int8_t *p, int8_t a, int8_t b,
                                          int8_t c, int8_t d) {
...
__forceinline__ __device__ uint32_t swap_half_wave_b32(uint32_t v) {
  return static_cast<uint32_t>(
      __builtin_amdgcn_permlanex16(static_cast<int>(v), static_cast<int>(v),
...
template <int Width> __forceinline__ __device__ float row_reduce_fmax(float v) {
#pragma unroll
...
template <int Rotation> __forceinline__ __device__ void convrot(float *v) {
  ...
  static_assert(Rotation == 4 || Rotation == 64 || Rotation == 128 ||
                    Rotation == 129,
...
__forceinline__ __device__ MmaInt8::Frag pack_prob_frag(const uint32_t p[8],
                                                        int lane) {
...
#pragma unroll
  for (int e = 0; e < 8; ++e)
    packed[e] = static_cast<OutT>(vals[e]);
...
__forceinline__ __device__ void load_o_tile(const InT *__restrict__ row,
                                            int d_base, float *vals, int lane) {
#if defined(COMFY_MMA_GFX12)
...
#pragma unroll
  for (int e = 0; e < 8; ++e)
    vals[e] = static_cast<float>(packed[e]);
#else
#pragma unroll
  for (int e = 0; e < 8; ++e) {
