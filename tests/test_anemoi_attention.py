...
"""Anemoi attention: reference, layout, routing, public-op, and native tests.
...
from comfy_kitchen.backends.eager.anemoi import (
    anemoi_attention,
    draft_probability,
    route_counts,
    route_probability,
...
def test_layout_matches_direct_construction(block, anchors):
    ...
    from comfy_kitchen.anemoi import make_ragged_2d_partition
...
def test_reference_proxy_finite(proxy):
    ...
    out = anemoi_attention(q, k, v, video_shape=(2, 5, 13), draftmap_proxy=proxy)
...
def test_public_options_reach_reference(options):
    ...
    actual = ck.anemoi_attention(q, q, q, video_shape=(2, 5, 13), **options)
...
def test_fake_op_output_metadata():
    ...
    with FakeTensorMode():
        ...
        out = ck.anemoi_attention(q, q, q, video_shape=(2, 5, 13), query_block_size=128)
...
def test_reference_runs_on_cuda(dim):
    ...
    with ck.use_backend("eager"):
        out = ck.anemoi_attention(q, q, q, video_shape=(2, 5, 13), query_block_size=64)
...
def test_native_sparse_pipeline_matches_reference(tied, anchors_enabled):
    ...
    actual = ck.anemoi_attention(q, k, v, **options)
...
def test_native_preserves_bhsd_input_storage(monkeypatch, block, dtype):
    ...
    expected = ck.anemoi_attention(*(x.contiguous() for x in (q, k, v)), **options)
    torch.testing.assert_close(actual, expected, atol=0, rtol=0)
