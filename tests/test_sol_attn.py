...
def _qkv(b, t, h, seed=0, device="cuda"):
    ...
    def mk(s):
        return (
            torch.randn(b, t, h, HD, device=device, dtype=torch.bfloat16, generator=g)
            * s
        )
...
def _bhnd_views(b, t, h, seed=3):
    ...
    views = tuple(
        torch.randn(b, h, t, HD, device="cuda", dtype=torch.bfloat16, generator=g)
        .mul_(0.5)
        .transpose(1, 2)
        for _ in range(3)
    )
...
def _chunked_case(seed, rot, v_scale=1.0):
    ...
    t, h, d = (
        ...
        4,
        HD,
    )  # last chunk is 100 tokens: ragged at the chunk AND 64-block level
    qkv = (
        torch.randn(t, 3 * h * d, device="cuda", dtype=torch.bfloat16, generator=g)
        * 0.5
    )
    qkv[:, 2 * h * d :] *= v_scale
...
def test_tau_direction():
    ...
    ref = _dense(q, k, v)
    assert (
        _cos(ck.sol_attn(q, k, v, tau=0.5), ref)
        > _cos(ck.sol_attn(q, k, v, tau=6.0), ref) + 5e-3
    )
...
def test_eager_refuses_video_length_rather_than_oom():
    ...
    q, k, v = (
        torch.empty(1, 37296, 56, HD, device="meta", dtype=torch.float16)
        for _ in range(3)
    )
...
def test_key_bias_bad_shape_rejected():
    ...
    with pytest.raises(ValueError, match="key_bias"):
        backend.sol_attn(q, k, v, tau=1.4, key_bias=torch.zeros(1, 128, device="cuda"))
...
def test_incapable_device_rejected_at_the_wrapper(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match=match):
        backend.sol_attn_chunked(
            ...
            256,
            4,
            torch.randn(1, 256, 1, 32, 2, 2, device="cuda"),
...
def test_topk_budget_direction():
    ...
    q, k, v = _qkv(1, 4096, 2)
    ref = _dense(q, k, v)
    assert _cos(sol_attn_eager(q, k, v, topk_ratio=0.05), ref) + 5e-3 < _cos(
        sol_attn_eager(q, k, v, topk_ratio=0.6), ref
    )
...
def test_fp16_strided_inputs_and_mixed_dtype():
    ...
    q16, k16, v16 = (
        x.half().permute(0, 2, 1, 3).contiguous().permute(0, 2, 1, 3) for x in (q, k, v)
    )
    got = ck.sol_attn(q16, k16, v16, tau=1.0)
    ref = ck.sol_attn(q, k, v, tau=1.0)
    rel = ((got.float() - ref.float()).norm() / ref.float().norm()).item()
    ...
    ws = torch.empty(
        ext.sol_attn_plan(1, 256, 1)["total"], dtype=torch.uint8, device="cuda"
    )
    ...
    with pytest.raises(RuntimeError, match="out"):
        ext.sol_attn(
            ...
            w(k1),
            w(v1),
            w(torch.empty_like(q1, dtype=torch.float16)),
            w(ws),
            *args,
        )
...
def _token_routing_case(t=4096 + 5, h=8, seed=7):
    ...
    q = bases[base_of_block].repeat_interleave(64, dim=0)[:t] + 0.5 * torch.randn(
        t, h, d, device="cuda", generator=g
    )
    ...
    k[idx] = (
        bases[torch.arange(8 * per_base, device="cuda") % 8] * gain[:, None, None]
        + 0.5 * k[idx]
    )
...
def test_token_aug_is_deterministic():
    ...
    qb, kb, vb = (
        x.permute(0, 2, 1, 3).contiguous().permute(0, 2, 1, 3) for x in (q, k, v)
    )
...
def test_token_aug_chunked_matches_direct():
    ...
    ref = backend.sol_attn(
        c["q"], c["k"], c["v"], tau=1.4, sink_blocks=[0, 2], token_aug=256
    )
    ...
    assert _rel(out, _dense(c["q"], c["k"], c["v"])) < _rel(
        plain, _dense(c["q"], c["k"], c["v"])
    )
...
def test_bindings_check_buffer_sizes():
    ...
    q, k, v = _qkv(1, t, h)
    ws = torch.empty(
        ext.sol_attn_plan(1, t, h)["total"], dtype=torch.uint8, device="cuda"
    )
    stream = torch.cuda.current_stream().cuda_stream
    ...
    with pytest.raises(RuntimeError, match="out must be contiguous"):
        ext.sol_attn(
            ...
            w(v),
            w(
                torch.empty(1, h, t, HD, device="cuda", dtype=torch.bfloat16).transpose(
                    1, 2
                )
            ),
            w(ws),
    ...
    with pytest.raises(RuntimeError, match="qkv"):
        ext.sol_producer_chunk(
            w(ws),
            w(torch.empty(64, 3 * h * HD - 8, device="cuda", dtype=torch.bfloat16)),
            w(torch.empty(t, 64, 2, device="cuda")),
            w(stats[0]),
            w(stats[0]),
            w(stats),
            w(stats),
    ...
    chunk, fab = torch.empty(
        64, 3 * h * HD, device="cuda", dtype=torch.bfloat16
    ), torch.empty(t, 64, 2, device="cuda")
...
def test_bindings_check_dtype_and_layout():
    ...
    with pytest.raises(RuntimeError, match="workspace must be a uint8"):
        ext.sol_attn(
            w(q), w(k), w(v), w(out), w(torch.zeros(ws.numel(), device="cuda")), *args
        )
    ...
    with pytest.raises(RuntimeError, match="out"):  # int8 out: half the bytes written
        ext.sol_attn_core(
            w(ws),
            w(torch.zeros(1, t, h, HD, device="cuda", dtype=torch.int8)),
            w(stats),
            w(stats),
            w(stats),
            1,
...
def test_bindings_reject_non_positive_extents(extents):
    ...
    q, k, v = _qkv(1, 256, 2)
    ws = torch.empty(
        ext.sol_attn_plan(1, 256, 2)["total"], dtype=torch.uint8, device="cuda"
    )
    b, t, h = extents
...
def test_zero_block_len_is_clamped_to_one_row():
    ...
    assert (
        _cos(got[:, ::64], sol_attn_eager(q, k, v, tau=1.4, block_len=ones)[:, ::64])
        > 0.998
    )
    ...
    kw = {
        "tau": 1.4,
        "kmean": torch.zeros(c["h"], HD, device="cuda"),
    ...
    from_zero = backend.sol_attn_chunked(
        c["chunks"], c["t"], c["h"], c["freqs"], c["norm"], block_len=zero_c, **kw
    )[0]
    assert torch.isfinite(from_zero.float()).all()
    assert torch.equal(
        from_zero,
        backend.sol_attn_chunked(
            c["chunks"], c["t"], c["h"], c["freqs"], c["norm"], block_len=one_c, **kw
        )[0],
    )
...
def test_chunked_producer_validates():
    ...
    with pytest.raises(ValueError, match="chunks cover"):
        backend.sol_attn_chunked(
            c["chunks"][:-1], c["t"], c["h"], c["freqs"], c["norm"]
        )
...
def test_chunked_zero_vscale_is_clamped():
    ...
    assert torch.isfinite(
        out.float()
    ).all()  # that step's V is sign-quantized: finite is the bar
...
@pytest.mark.parametrize(
    "t", [64 * 40, 1000]
)  # 1000: ragged tail, block_len[-1] clamped
@pytest.mark.parametrize("select", [{"tau": 1.4}, {"topk_ratio": 0.2}])
def test_block_len_matches_eager(t, select):
    ...
    assert torch.equal(
        ck.sol_attn(q2, k2, v2, block_len=block_len, **select)[:, valid], got[:, valid]
    )
...
def _masked_dense(q, k, v, valid):
    ...
    out = torch.nn.functional.scaled_dot_product_attention(
        qq, kk, vv, attn_mask=mask, scale=HD**-0.5
    )
    return out.permute(0, 2, 1, 3)
...
def test_no_tail_matches_eager():
    ...
    dense = ck.sol_attn(
        q, k, v, tau=1.4, tail=False, block_len=block_len, sink_q=[0, 30]
    )
...
def _coarse_reference(q, k, v, valid, block_len, gate, scale=HD**-0.5):
    ...
    means = [
        torch.stack(
            [xx[:, 64 * i : 64 * i + 64].sum(1) / lengths[i] for i in range(n)], 1
        )
        for xx in x
    ]  # 3 x (B, N, H, D)
...
def test_coarse_gate_matches_eager():
    ...
    assert (
        _cos_rows(got, _coarse_reference(q, k, v, valid, block_len, gate), valid)
        > 0.9999
    )
    assert (
        _cos_rows(got, _masked_dense(q, k, v, valid), valid) < 0.999
...
def test_topk_budget_excludes_sinks():
    ...
    for sinks, n_eff in (((0, 16), 16), ((28, 40), 28), ((0, 0), 32)):
        ...
        assert (
            (s >= thr.unsqueeze(-1)).sum(-1).eq(kk).all()
        ), sinks  # exactly kk non-sink blocks kept
...
def test_chunked_vsa_mode():
    ...
    block_len = torch.randint(32, 65, ((t + 63) // 64,), device="cuda", generator=g).to(
        torch.int32
    )
    ...
    out, _, _ = backend.sol_attn_chunked(
        c["chunks"], t, h, c["freqs"], c["norm"], kmean=km, vscale=vs, **kw
    )
    assert torch.isfinite(out.float()).all()
...
def test_block_len_validation():
    ...
    with pytest.raises(ValueError, match="block_len"):
        backend.sol_attn_chunked(
            c["chunks"], c["t"], c["h"], c["freqs"], c["norm"], block_len=bad_dtype
        )
